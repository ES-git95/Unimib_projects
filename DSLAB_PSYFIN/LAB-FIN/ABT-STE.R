TStickers=read.csv("C:\\Users\\preinstalled\\Desktop\\Big_Data_in_economics\\TS_tickers.csv")
coppie=read.csv("C:\\Users\\preinstalled\\Desktop\\Big_Data_in_economics\\coppie.csv")
library(riingo)
library(quantmod)
library(egcm) 
library(ggplot2)
...
Tr <- nrow(Y_)
T_start <- round(0.3*Tr)
T_lookback <- 1000  # lookback window length
T_shift <- 20  # how often is refreshed
# init empty variables
gamma_rolling_LS <- mu_rolling_LS <- xts(rep(NA, Tr), index(Y_))
colnames(mu_rolling_LS) <- "mu-rolling-LS"
colnames(gamma_rolling_LS) <- "gamma-rolling-LS"
# loop
t0_update <- seq(from = min(T_start, T_lookback), to = Tr-T_shift, by = T_shift)
for (t0 in t0_update) {
  T_lookback_ <- ifelse(t0-T_lookback+1 >= 1, T_lookback, T_start)
  ls_coeffs <- coef(lm(Y_[(t0-T_lookback_+1):t0, 1] ~ Y_[(t0-T_lookback_+1):t0, 2],
                       weights = last(1:T_lookback, T_lookback_)))
  mu_rolling_LS[t0+1] <- ls_coeffs[1]
  gamma_rolling_LS[t0+1] <- ls_coeffs[2]
}

dim(gamma_rolling_LS)
